...
singleNoBet = data.bet_amount_tiger;
    doubleNoBet = data.bet_amount_tie;
    tripleNoBet = data.bet_amount_elep;

    var array=[]
  array.push(data.bet_amount_tiger)
  array.push(data.bet_amount_tie)
  array.push(data.bet_amount_elep)
  isbetPlaced=true
  ...
  Win_singleNo=array.indexOf(Math.min(...array))
    ...
    let point = await service.getUserPoint(
      data.playerId,
      data.bet_amount_tiger + data.bet_amount_tie + data.bet_amount_elep
    );
    console.log("Point", point);
    //if (data.betValue == 0) {
    if (
      data.bet_amount_tiger == 0 &&
      data.bet_amount_tie == 0 &&
      data.bet_amount_elep == 0
    ) {
      socket.emit(events.OnBetsPlaced, {
        ...
        data: {
          ...
          balance:
            point -
            (data.bet_amount_tiger +
              data.bet_amount_tie +
              data.bet_amount_elep),
        },
      });
    } else {
      socket.emit(events.OnBetsPlaced, {
...
async function WinNosCalculator() {
  ...
  if (singleNoBet != 0 && Win_singleNo == 0) {
    winPoint = winPoint + singleNoBet * 2; //******** tiger
  }

  if (doubleNoBet != 0 && Win_singleNo == 1) {
    // winPoint = winPoint + singleNoBet * 9; //********tie
    winPoint = winPoint + doubleNoBet * 
    10; //********tie
  }
  if (tripleNoBet != 0 && Win_singleNo == 2) {
    //  winPoint = winPoint + singleNoBet * 2; //
    winPoint = winPoint + tripleNoBet * 2; //********** elephant
  }
  singleNoBet = 0;
  doubleNoBet = 0;
  tripleNoBet = 0;
...
module.exports.StartDVsTGame = StartDVsTGame;
