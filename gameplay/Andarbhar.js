...
function WinNosCalculator() {
  ...
  let CardsLength =
    winSpot[0] === spot.Andar
      ? getRandomEvenNo(CardRange)
      : getRandomOddNo(CardRange);

  for (let i = 0; i < CardsLength; i++) {
    // create Aandar bhar cards
  ...
  for (let i = 0; i < BotsBetsDetails.length; i++) {
    ...
    winningSpot.forEach((winSpot) => {
      ...
      BotsBetsDetails[i].balance +=
        BotsBetsDetails[i][spot] * winningRate[winSpot];
      win += BotsBetsDetails[i][spot] * winningRate[winSpot];
    });

    if (win === 0) {
      BotsBetsDetails[i].win = -(
        BotsBetsDetails[i].Andar +
        BotsBetsDetails[i].Bahar +
        BotsBetsDetails[i].oneToFive +
        BotsBetsDetails[i].sixToTen +
        BotsBetsDetails[i].elevenToFifteen +
        BotsBetsDetails[i].sixteenToTwentyFive +
        BotsBetsDetails[i].twentySixToThirty +
        BotsBetsDetails[i].thirtyOneToThirtyFive +
        BotsBetsDetails[i].thirtySixToFouty +
    ...
    for (let i = 0; i < MAX_ITERATION; i++) {
      ...
      switch (spot) {
        ...
        case 3:
          ...
          break;
        case 4:
          _elevenToFifteenBets += chip;
          break;
        case 5:
          _sixteenToTwentyFiveBets += chip;
          break;
        case 6:
          _twentySixToThirtyBets += chip;
          break;
        case 7:
          _thirtyOneToThirtyFiveBets += chip;
          break;
        case 8:
          _thirtySixToFoutyBets += chip;
          break;
        case 9:
          _fortyOneAndMoreBets += chip;
          break;
        default:
          break;
      }
...
function ResetTimers() {
  ...
  service.saveJokerCard({
    joker_card_no: Joker_Card_No,
    room_id: timeStamp,
    game_id: gameId,
  }); //db
...
function OnTest(data) {
  ...
  socket.on(commonVar.test, (data) => {
    socket.emit(commonVar.test, {
      timer: timerVar.percardDisplayTime,
      MAX_ITERATION,
    });
