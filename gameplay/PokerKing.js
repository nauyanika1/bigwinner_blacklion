...
let timer1 = timerVar.bettingTimerPokerKing;
...
const LEFT_WIN_RATE = 1.9;
const RIGHT_WIN_RATE = 3.8;
const MIDDLE_WIN_RATE = 20;
...
async function OnPlayerWin() {
  ...
  for (let socketId in BetHolder) {
    ...
    if (winningSpot === 0) {
      betData[commonVar.win] = betData[0] * LEFT_WIN_RATE;
    } else if (winningSpot === 1) {
      betData[commonVar.win] = betData[1] * MIDDLE_WIN_RATE;
    } else {
      betData[commonVar.win] = betData[2] * RIGHT_WIN_RATE;
    }
...
function WinNosCalculator() {
  let totalLeftBets = SumOfARRAY(LeftBets) * 1.9;
  let totalMiddleBets = SumOfARRAY(MiddleBets) * 20;
  let totalRightBets = SumOfARRAY(RightBets) * 3.8;
  ...
  Win_singleNo = Math.floor(Math.random() * 54);
  Win_doubleNo = Math.floor(Math.random() * 54);
  Win_jokerNo = Math.floor(Math.random() * 54);
  let con_doubleNo = parseInt(
    Win_doubleNo.toString() + Win_singleNo.toString()
  );
  let con_jokerNo = parseInt(
    Win_jokerNo.toString() + Win_doubleNo.toString() + Win_singleNo.toString()
  );
  winPoint = 0;
  if (usersingleNoChoice.indexOf(Win_singleNo) != -1) {
    winPoint =
      winPoint + singleNoBet[usersingleNoChoice.indexOf(Win_singleNo)] * 1.9;
  }
  if (userdoubleNoChoice.indexOf(con_doubleNo) != -1) {
    winPoint =
      winPoint + doubleNoBet[userdoubleNoChoice.indexOf(con_doubleNo)] * 3.8;
  }
  if (userjokerNoChoice.indexOf(con_jokerNo) != -1) {
    winPoint =
      winPoint + jokerNoBet[userjokerNoChoice.indexOf(con_jokerNo)] * 20;
  }
  return { winNo: winNo, win_point: winPoint, spot: leastBetSpot };
}
...
function CalculateBotsWinAmount(winningSpot) {
  for (let i = 0; i < BotsBetsDetails.length; i++) {
    ...
    let win = 0;
    if (winningSpot == 0) {
      BotsBetsDetails[i].win = BotsBetsDetails[i].left * LEFT_WIN_RATE;
      BotsBetsDetails[i].balance += BotsBetsDetails[i].left * LEFT_WIN_RATE;
      win = BotsBetsDetails[i].left * LEFT_WIN_RATE;
    } else if (winningSpot === 1) {
      ...
      BotsBetsDetails[i].win = BotsBetsDetails[i].right * RIGHT_WIN_RATE;
      BotsBetsDetails[i].balance += BotsBetsDetails[i].right * RIGHT_WIN_RATE;
      win = BotsBetsDetails[i].right * RIGHT_WIN_RATE;
    }
