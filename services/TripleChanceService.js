...
if(user[0]['password']==password){
        ...
        let query = await db.query(sql,{to:receiverplayerId,from:senderplayerId,point:point})
       ...
       // password==senderplayerId.password
        ...
        sql = `UPDATE users SET point= ? WHERE email= ? `;
        savePoint  = await db.query(sql,[result-point,senderplayerId]);
       console.log("databaseupdated",savePoint)
...
module.exports = {JoinGame, lastWinningNo ,getRoundCount ,updateUserPoint, updateWinningNo ,onSendPointsToPlayer, updateWinningAmount,getUserBalance,getUserPoint,gameMartixRecords,gameSlotRecords}